from dns import resolver  # pip install dnspython
        ...
        try:
            ...
            for data in search:
                if record_type == "cname":
                    records.append("www" f" 14400s {str(record_type).upper()} {data}")
                else:
                    records.append(f" 14400s {str(record_type).upper()} {data}")
        except:
            raise Exception(f"Failed to search the record type {record_type}")
    return records
